...
def _compute_local_metrics(input: Tensor, enhanced: Tensor) -> LocalMetrics:
    mean_filter = partial(conv, kernel=_get_gaussian_kernel())

    input_patch_mean = mean_filter(input)
    enhanced_patch_mean = mean_filter(enhanced)

    input_patch_variance = mean_filter(input**2) - input_patch_mean**2
    enhanced_patch_variance = mean_filter(enhanced**2) - enhanced_patch_mean**2

    patch_covariance = (
        mean_filter(input * enhanced) - input_patch_mean * enhanced_patch_mean
    )
