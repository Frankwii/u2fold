...
def test_split_data_to_tuple():
    input = SplitData(training="foo", validation="bar", test="baz")
...
def test_split_data_map_without_params():
    ...
    output = input.map(lambda s: s.capitalize())

    expected = SplitData(training="Foo", validation="Bar", test="Baz")

    assert expected == output


def test_split_data_map_with_params():
    input = SplitData(0.1, 0.2, 0.3)

    multipliers = SplitData(10, 5, 10 / 3)

    output = input.map(lambda x, y: x * y, params=multipliers)
