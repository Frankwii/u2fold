...
class TemporaryDir(AbstractContextManager):
    def __init__(self, chosen_name: str):
        ...
        if not bool(chosen_name) or '/' in chosen_name or '.' in chosen_name:
            raise ValueError("chosen_name must be a valid, simple directory name.")
...
def check_numeric_tensor_collection_equality(col1: Collection[torch.Tensor], col2: Collection[torch.Tensor]) -> bool:
